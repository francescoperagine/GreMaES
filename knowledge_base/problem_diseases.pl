...
% black_spot
condition(black_spot) :- symptom(leaves, chlorotic), symptom(leaves, early_fall).
condition(black_spot) :- symptom(upper_side_leaves, spots, black).
...
% leaf_spot
condition(leaf_spot) :- symptom(leaves, visible_spores), symptom(leaves, spots, brown), symptom(leaves, spots, dark_margin).
condition(leaf_spot) :- symptom(leaves, visible_spores), symptom(leaves, spots, tan), symptom(leaves, spots, dark_margin).
condition(leaf_spot) :- symptom(leaves, visible_spores), symptom(leaves, spots, reddish), symptom(leaves, spots, dark_margin).
condition(leaf_spot) :- symptom(leaves, angular_lesions).
condition(leaf_spot) :- symptom(leaves, angular_lesions), symptom(leaves, altered_color, yellowish_outline).
condition(leaf_spot) :- symptom(leaves, circular_lesions).
condition(leaf_spot) :- symptom(leaves, circular_lesions), symptom(leaves, altered_color, yellowish_outline).
condition(leaf_spot) :- symptom(leaves, necrotic_lesions).
condition(leaf_spot) :- symptom(leaves, necrotic_lesions), symptom(leaves, altered_color, yellowish_outline).
...
% powdery_mildew
condition(powdery_mildew) :- symptom(all, white_powdery_substance).
...
condition(downy_mildew) :- symptom(lower_side_leaves, white_cotton_like_downy_substance).
condition(downy_mildew) :- symptom(lower_side_leaves, greysh_cotton_like_downy_substance).
condition(downy_mildew) :- symptom(lower_side_leaves, grayish_fuzzy_looking_spores).
...
condition(blight) :- symptom(all, spot).
condition(blight) :- symptom(all, withering).
condition(blight) :- symptom(all, death_of_plant_tissue).
...
% canker
condition(canker) :- symptom(leaves, small_holes).
condition(canker) :- symptom(bark, dead_patches).
...
condition(shot_hole) :- symptom(leaves, spots_drop_out).
condition(shot_hole) :- symptom(leaves, shot_holes).
condition(shot_hole) :- symptom(leaves, spots, purplish).
condition(shot_hole) :- symptom(leaves, spots_drop_out).
condition(shot_hole) :- symptom(leaves, shot_holes).
condition(shot_hole) :- symptom(fruits, spots, red).
condition(shot_hole) :- symptom(fruits, spots, red), symptom(fruits, clear_gummy_substance).
condition(shot_hole) :- symptom(fruits, spots, purplish).
condition(shot_hole) :- symptom(fruits, spots, purplish), symptom(fruits, clear_gummy_substance).
...
condition(grey_mold) :- symptom(stem, water_soaked_spots).
condition(grey_mold) :- symptom(flower, water_soaked_spots).
condition(grey_mold) :- symptom(lower_leaves, water_soaked_spots).
...
condition(verticilium_wilt) :- symptom(branches, wilting).
condition(verticilium_wilt) :- symptom(branches, dying).
